	...
	<tbody>
	 <%	
		...
		DepartmentsDAO departmentsDAO = new DepartmentsDAO();
	 	ArrayList<Departments> departments = departmentsDAO.getAll();
		//显示得到的所有数据
		for(int i=0;i<departments.size();i++){
			Departments department = departments.get(i);
	  %>
			<tr>
			  <td><%=department.getDepartments_id()%></td>
			  <td><%=department.getDepartments_name() %></td>
			  <td><%=department.getDepartments_TNum()%></td>
			  <td><%=department.getDepartments_SNum()%></td>
			  <td><%=department.getDepartments_address()%></td>
	                    
			  <td>
				<a href="<%=request.getContextPath() %>/pages/admin/InformationManagement/DepartmentManagement/modify.jsp?departments_id=<%=department.getDepartments_id() %>"><button class="button border-main" type="button"><span class="icon-edit"></span>修改</button></a>
